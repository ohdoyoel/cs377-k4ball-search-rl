"""Train SAC with the scoring-potential RM bonus on top of env reward.

Same hyper-parameters as ``run_inning_random.py`` (plain SAC, random
start, no aim/shape). The only change is that the reward channel is
augmented by lambda * RM(s_{t+1}) via ``ScoringPotentialRMEnv``.

Evaluation uses the original env reward (no RM bonus) on both canonical
and random-start distributions, so the headline numbers are directly
comparable to the Plain RL table.
"""
...
from billiards.wrappers.scoring_potential_rm_env import ScoringPotentialRMEnv  # noqa: E402
...
def make_train_env(seed: int, max_shots: int, rm_path: str, lam: float):
    ...
    env = ScoringPotentialRMEnv(env, rm_path=rm_path, lam=lam)
...
def main():
    ...
    parser.add_argument("--rm_path", type=str,
                        default="experiments/rm_data/scoring_potential_rm.pt")
    parser.add_argument("--lam", type=float, default=10.0,
                        help="Reward = score + lam * RM(s')")
    parser.add_argument("--out_dir", type=str,
                        default="experiments/runs_inning_random/sac_sprm_200k_s0")
    ...
    with contextlib.redirect_stdout(tee), contextlib.redirect_stderr(err_tee):
        ...
        print(f"[sac+rm] seed={args.seed} total_steps={args.total_steps} "
        ...
        print(f"[sac+rm] train done in {wall:.1f}s")
        ...
        for ev in ("canonical", "random"):
            ...
            print(f"[sac+rm] eval[{ev}] mean={r['mean_inning_score']:.3f} "
                  f"max={r['max_inning_score']} p>=1={r['p_score_ge1']:.1f}% "
                  f"foul%={r['foul_rate']:.1f}")
        with (out_dir / "summary.json").open("w") as f:
            json.dump(results, f, indent=2)
        print(f"[sac+rm] DONE wall={time.perf_counter()-t0:.1f}s")
